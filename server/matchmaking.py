...
async def _play_matchmaking(websocket, matchmaker, matchmaking,
    ...
    """Enqueue `username` in `matchmaker` (Play) and wait for
    server.tick's own tick loop to either pair them or time them out.
    ...
    threaded from server.composition, the same shape `clients` has for joined
